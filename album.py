...
class AlreadyExists(Error):
    '''вызывается, если альбом уже есть в базе'''
    pass

class Album(Base):
    '''описывает, как сделана таблица'''
...
def find(artist):
    """
    Находит все альбомы, по заданному артисту
    """
    session = connect_db()
...
def save(year, artist, genre, album):
    ...
    #После проверок сохраняем данные в БД
    # создаем объект класса Album, привязываем
    #к сессии и сохраняем.
    session.add(album)
    session.commit()
    return album
